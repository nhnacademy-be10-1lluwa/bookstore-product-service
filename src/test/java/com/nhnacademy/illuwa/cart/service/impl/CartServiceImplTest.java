...
class CartServiceImplTest {
    ...
    void testGetCart_withItems() {
        ...
        book.setBookImages(Collections.emptyList());    // null 방지
    ...
    void testAddBook_insufficientStock() {
        ...
        book.setBookImages(Collections.emptyList());
    ...
    void testAddBook_new() {
        ...
        Book book = new Book();
        book.setId(bookId);
        BookExtraInfo info = new BookExtraInfo();
        info.setCount(10);
    ...
    @Test
    @DisplayName("updateBookCart: 수량 차감 후 저장")
    void testUpdateBookCart_partialDeduction() {
        Cart cart = new Cart(memberId);
        when(cartRepository.findByMemberId(memberId)).thenReturn(Optional.of(cart));
        ...
        BookCart bc = new BookCart(cart, book, 4);
        when(bookCartRepository.findByCartAndBook(cart, book))
                .thenReturn(Optional.of(bc));

        BookCartResponse resp = cartService.updateBookCart(
                new BookCartRequest(memberId, bookId, 2)
        );

        verify(bookCartRepository, never()).delete(any());
        verify(bookCartRepository).save(argThat(updated ->
                updated.getAmount() == 2 &&
                        updated.getCart()   == cart &&
                        updated.getBook()   == book
        ));
        assertEquals(2, resp.getAmount());
    }

    @Test
    @DisplayName("updateBookCart: 요청 수량이 전체 수량 이상이면 삭제")
    void testUpdateBookCart_deleteWhenZeroOrLess() {
        Cart cart = new Cart(memberId);
        when(cartRepository.findByMemberId(memberId)).thenReturn(Optional.of(cart));
        ...
        BookCartResponse resp = cartService.updateBookCart(
                new BookCartRequest(memberId, bookId, 3)
        );
    ...
    @Test
    @DisplayName("removeBookCart: 요청 수량과 무관하게 전체 삭제")
    void testRemoveBookCart_success() {
        Cart cart = new Cart(memberId);
        when(cartRepository.findByMemberId(memberId)).thenReturn(Optional.of(cart));

        Book book = new Book();
        book.setBookExtraInfo(new BookExtraInfo(Status.NORMAL, false, 5));
        when(bookRepository.findById(bookId)).thenReturn(Optional.of(book));

        BookCart bc = new BookCart(cart, book, 5);
        when(bookCartRepository.findByCartAndBook(cart, book))
                .thenReturn(Optional.of(bc));

        cartService.removeBookCart(
                new BookCartRequest(memberId, bookId, 1)
        );
